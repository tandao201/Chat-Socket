...
//				.createTitledBorder(String.format("Server %s (%s)", Main.socketController.connectedServer.nickName,
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					if (foundRoom == null) {
						...
						System.out.println("Clicker không có room");
					} else {
						System.out.println("Clicker có room");
						int roomTabIndex = -1;
...
//					String roomIdStr = "";
//					roomIdStr += clickedUser+","+Main.socketController.userName;
//					Main.socketController.loadMessage(roomIdStr);
				}
			}
		});
		...
		fileButton = new JButton(Main.getScaledImage("D:/Chat_App_Project/ChatApp/ChatClient/resources/fileIcon.png", 16, 16));
	...
	private void addNewMessageGUI(int roomID, MessageData messageData) {
		...
		Color partnerColor = Color.lightGray;
		JPanel panel = new JPanel();
		panel.setMaximumSize(new Dimension(0,10));
		MessagePanel newMessagePanel = new MessagePanel(messageData);


		newMessagePanel.validate();
		newMessagePanel.repaint();
		if (messageData.whoSend.equals(Main.socketController.userName)) {
			newMessagePanel.setBorder(BorderFactory.createLineBorder(meColor));
			newMessagePanel.setBackground(meColor);
			newMessagePanel.setAlignmentX(Component.LEFT_ALIGNMENT);
		} else {
			newMessagePanel.setBorder(BorderFactory.createLineBorder(partnerColor));
			newMessagePanel.setBackground(partnerColor);
			newMessagePanel.setAlignmentX(Component.RIGHT_ALIGNMENT);
...
//		receiveMessageRoomMessagesPanel.add(Box.createHorizontalGlue());
		receiveMessageRoomMessagesPanel.add(panel);
	...
	public static class AudioButton extends JButton implements ActionListener {
		...
		public AudioButton() {
			microphoneImage = Main.getScaledImage("D:/Chat_App_Project/ChatApp/ChatClient/resources/microphone.png", 16, 16);
			stopImage = Main.getScaledImage("D:/Chat_App_Project/ChatApp/ChatClient/resources/stop.png", 16, 16);
